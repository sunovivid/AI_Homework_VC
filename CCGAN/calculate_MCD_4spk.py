...
sourceDir = '../seoulNarrative_allSpk_v2/testSet'
sourceWavs = {}
...
outFolder = './logs_MCD/'
resultFileName_base = outFolder+'MCD_result_'
...
def calculateMCD_manual_mfcc(target_converted_file_path, compare_original_file_key, target_converted_source_id, compare_original_target_id):
    ...
    distance, _cost_matrix, _acc_cost_matrix, _path = dtw(target_conv_wav_mfcc[:,1:], compare_ori_wav_mfcc[:,1:], dist=dist)
    '''def CalculateDistance(path, a, b):
        val=0
        for each in path:
            val += (a[each[0]]-b[each[1]])**2
        return np.sum(val)/len(path)
    distance = CalculateDistance(path, target_conv_wav_mfcc[:,1:], compare_ori_wav_mfcc[:,1:])'''

    #mcd = (np.divide(10.0, np.log(10)) * np.sqrt(2*(distance)))
    mcd = np.divide(10.0, np.log(10)) * (np.sqrt(2) * distance)
