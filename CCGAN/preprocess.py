...
def load_wavs(wav_dir, sr):
    ...
    wavs = list()
    file_num = len(os.listdir(wav_dir))
    #file_num_mod = file_num//4
    file_num_mod = 200
    print("use {} files per spk".format(file_num_mod))
    file_list_mod = os.listdir(wav_dir)[:file_num_mod]
    
    for file in file_list_mod:
...
def world_encode_data(wavs, fs, frame_period = 5.0, coded_dim = 24):
    ...
    for wav in wavs:
        ...
        if timeaxis.shape[0] >= 128:
            ...
            print("too short sp is ignored: {}".format(timeaxis.shape[0]))
...
def sample_train_data_mix_all_mapping(dataset_list, n_frames = 128):
    ...
    for speaker in range(num_speakers):
        ...
        mixed_dataset_list_src.append(train_data_src) #10 162 128 24
        mixed_dataset_list_tar.append(train_data_tar) #10 162 128 24
        
    #mixed_dataset_list_src = list(map(list, zip(*mixed_dataset_list_src)))
    #mixed_dataset_list_src = np.array(mixed_dataset_list_src) #162 10 128 24
    
    #mixed_dataset_list_tar = list(map(list, zip(*mixed_dataset_list_tar)))
    #mixed_dataset_list_tar = np.array(mixed_dataset_list_tar) #162 10 128 24
...
def sample_train_data_mix_mapping(src_spk_data, tar_spk_data, num_samples, n_frames = 128):
    #for reduce memory consume, just use target / pick speaker
    ...
    for idx in train_data_idx_src:
        data = src_spk_data[idx] #shape: (mfcc_dim<24>, length)
        frames_total = data.shape[1]
        assert frames_total >= n_frames
        start = np.random.randint(frames_total - n_frames + 1)
        end = start + n_frames 
    ...
    #train_data_src = np.moveaxis(train_data_src, -1, 1)
            
    train_data_tar = np.array(train_data_tar) 
    #train_data_tar = np.moveaxis(train_data_tar, -1, 1)
    
    return train_data_src, train_data_tar 
