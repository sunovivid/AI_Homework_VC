...
vis7 = visdom.Visdom()
vis8 = visdom.Visdom()
plot_train_rec = vis1.line(Y=torch.tensor([0]),X=torch.tensor([0]),opts=dict(legend=['train','dev']),)
plot_train_kl = vis2.line(Y=torch.tensor([0]),X=torch.tensor([0]),opts=dict(legend=['train','dev']),)
plot_train_cyc = vis3.line(Y=torch.tensor([0]),X=torch.tensor([0]),opts=dict(legend=['train','dev']),)
plot_cur_loss = vis7.line(Y=torch.tensor([0]),X=torch.tensor([0]),opts=dict(legend=['train','dev']),)
...
SP_DICT_TRAIN, SP_DICT_DEV = split_train_dev(SP_DICT, train_percent=0)
...
if is_MD:
    ...
    for spk_id in SPK_DICT.values():
        ...
        Dec_group[spk_id].load_state_dict(torch.load(model_dir+"/final_"+spk_id+"_dec.pt"))
...
for epoch in range(epochs):
    ...
    # Visualization
    vis1.line(Y=[lm.get_stat("rec_loss")], X=np.array([epoch]), name='train_rec_loss', win=plot_train_rec, update='append')
    vis2.line(Y=[lm.get_stat("kl_loss")], X=np.array([epoch]), name='train_kl_loss', win=plot_train_kl, update='append')
    if is_cyc:
        vis3.line(Y=[lm.get_stat("cyc_loss")], X=np.array([epoch]), name='train_cyc_loss', win=plot_train_cyc, update='append')
    ...
    # Visualization
    vis4.line(Y=[lm.get_stat("rec_loss")], X=np.array([epoch]), name='dev_rec_loss', win=plot_train_rec, update='append')
    vis5.line(Y=[lm.get_stat("kl_loss")], X=np.array([epoch]), name='dev_kl_loss', win=plot_train_kl, update='append')
    if is_cyc:
        vis6.line(Y=[lm.get_stat("cyc_loss")], X=np.array([epoch]), name='dev_cyc_loss', win=plot_train_cyc, update='append')
    end_time = time.time()
    ...
    if epoch % 5 == 0:
        ...
        if is_cyc:
            ...
            cur_loss = lm.get_stat("rec_loss") + 3
        ...
        if min_dev_loss > cur_loss:
            ...
            min_epoch = epoch
        # Visualization
        vis7.line(Y=[cur_loss], X=np.array([epoch]), name='loss', win=plot_cur_loss, update='append')
        vis8.line(Y=[min_dev_loss], X=np.array([epoch]), name='min_loss', win=plot_cur_loss, update='append')
