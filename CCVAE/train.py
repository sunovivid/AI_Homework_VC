...
coef_dict={ 
    ...
    "GAN": {"rec": 3.0, "cyc": 1.0, "adv": 1.0, "kl": 1.0},
    "CC": {"rec": 10.0, "cyc": 5.0, "adv": 1.0, "kl": 1.0},
    "GAN_CC": {"rec": 10.0, "cyc": 5.0, "adv": 1.0, "kl": 1.0},
    "MD_CC": {"rec": 3.0, "cyc": 10.0, "adv": 1.0, "kl": 1.0},
    "MD_GAN_CC": {"rec": 5.0, "cyc": 10.0, "adv": 1.0, "kl": 1.0}
}

if args.GAN and args.CC:
    if args.model_type == "MD":
        coef = coef_dict["MD_GAN_CC"]
    else:
        coef=coef_dict["GAN_CC"]
elif args.GAN and not args.CC:
    coef=coef_dict["GAN"]
elif args.CC and not args.GAN:
    if args.model_type == "MD":
        # lr = 0.001
...
if args.GAN:
    if args.model_type == "MD": 
        ...
        D_opt_group=dict()
        for spk_id in SPK_DICT.values():
            D_group[spk_id] = model.Discriminator(style_dim=4, vae_type=args.model_type)
            D_group[spk_id].cuda()
            D_opt_group[spk_id] = optim.Adam(D_group[spk_id].parameters(), lr=lr)
    else:
        D = model.Discriminator(style_dim=4, vae_type=args.model_type)
...
for epoch in range(epochs):
    ...
    is_gan = args.GAN and coef["adv"] > 0.0
    is_cyc = args.CC and coef["cyc"] > 0.0
    if epoch % 500 == 0:
        print("Batch Size",batch_size)
        print("GAN training:", is_gan)
    ...
    if is_gan:
        ...
        for D_epoch in range(d_epoch):
            for A_spk, B_spk in spk_pair:
                ...
                if is_MD:
                    Dec_A = Dec_group[A_spk]; D_A = D_group[A_spk]
                    Dec_B = Dec_group[B_spk]; D_B = D_group[B_spk]
                    D_opt_A = D_opt_group[A_spk]
                    D_opt_B = D_opt_group[B_spk]
                    
                else:
                    Dec_A = Dec; D_A = D
                    Dec_B = Dec; D_B = D
                    D_opt_A = D_opt; D_opt_B = D_opt
                ...
                for (A_x, A_y), (B_x, B_y) in zip(A_loader, B_loader):
                    ...
                    _, _, A2B = Dec_B(A_z, B_y)
                    _, _, B2A = Dec_A(B_z, A_y)
                    _, _, A2A = Dec_A(A_z, A_y)
                    _, _, B2B = Dec_B(B_z, B_y)

                    
                    true_prob = torch.mean(D_A(A_x, A_y)) + torch.mean(D_B(B_x, B_y))
                    gen_prob = torch.mean(D_B(A2B, B_y)) + torch.mean(D_A(B2A, A_y))
                    # true_prob = torch.mean(D_A(A_x, A_y)) + torch.mean(D_B(B_x, B_y))
                    gen_prob += torch.mean(D_B(B2B, B_y)) + torch.mean(D_A(A2A, A_y))

                    adv_loss = 2*true_prob - gen_prob
                    if is_MD:
                        update_parm([D_opt_A, D_opt_B], -adv_loss)
                    else:   
                        update_parm([D_opt], -adv_loss)
                    lm.add_torch_stat("D_adv_loss", -adv_loss)
    ...
    for A_spk, B_spk in spk_pair:
        ...
        if is_MD:
            ...
            Dec_A = Dec
            Dec_B = Dec
            Dec_opt_A = Dec_opt; Dec_opt_B = Dec_opt
            if is_gan:  
                D_A = D
                D_B = D
        ...
        for (A_x, A_y), (B_x, B_y) in zip(A_loader, B_loader):
            ...
            if is_gan:
                true_prob = torch.mean(D_A(A_x, A_y)) + torch.mean(D_B(B_x, B_y))
                gen_prob = torch.mean(D_B(A2B, B_y)) + torch.mean(D_A(B2A, A_y))
                gen_prob += torch.mean(D_A(A2A, A_y)) + torch.mean(D_B(B2B, B_y))

                adv_loss = 2*true_prob - gen_prob
                total_loss += coef["adv"] * adv_loss
            ...
            if is_gan:
                lm.add_torch_stat("G_adv_loss", adv_loss)
            if is_cyc:
